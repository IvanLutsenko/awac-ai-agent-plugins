...
- **combined-review** (v1.10.1) — Multi-agent code review + CodeRabbit CLI; GitHub PR + GitLab MR with inline resolvable threads on both; `/rereview` verifies + resolves + approves on both; opt-in `security-reviewer`; `/review-config` for language/model/CodeRabbit/security
