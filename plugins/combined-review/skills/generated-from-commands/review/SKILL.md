...
`plugins/combined-review/commands/review-config.md` and follow it, then continue this review with the
...
- Codex cannot choose a model per sub-agent: run them on the current model and say so in the report instead of naming a model that never ran.
