...
export type ToolArgs = Record<string, any> | undefined;
export type Handler = (vaultPath: string, args: ToolArgs) => Promise<ToolResult>;
...
export async function listProjects(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
...
export async function getProject(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
...
export async function createProject(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
  if (!args) throw new Error("Missing arguments");
  const projectName = sanitizeTitle(args.name as string);
...
export async function addBug(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
  if (!args) throw new Error("Missing arguments");
  const projectPath = await resolveProjectPath(vaultPath, args.project as string);

  const title = sanitizeTitle(args.title as string);
...
export async function addSession(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
  if (!args) throw new Error("Missing arguments");
  const projectPath = await resolveProjectPath(vaultPath, args.project as string);
...
export async function search(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
  if (!args) throw new Error("Missing arguments");
...
export async function archiveProject(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
  if (!args) throw new Error("Missing arguments");
...
export async function restoreProject(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
  if (!args) throw new Error("Missing arguments");
  const projectName = args.project as string;
...
export async function deleteProject(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
  if (!args) throw new Error("Missing arguments");
  const projectName = args.project as string;
...
export async function closeBug(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
  if (!args) throw new Error("Missing arguments");
  const projectPath = await resolveProjectPath(vaultPath, args.project as string);
...
export async function addTask(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
  if (!args) throw new Error("Missing arguments");
  const projectPath = await resolveProjectPath(vaultPath, args.project as string);
...
export async function updateTask(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
  if (!args) throw new Error("Missing arguments");
  const projectPath = await resolveProjectPath(vaultPath, args.project as string);
...
export async function listTasks(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
  if (!args) throw new Error("Missing arguments");
  const projectPath = await resolveProjectPath(vaultPath, args.project as string);
...
export async function deleteTask(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
  if (!args) throw new Error("Missing arguments");
  const projectPath = await resolveProjectPath(vaultPath, args.project as string);
  const taskId = args.taskId as number;
...
export async function updateProject(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
  if (!args) throw new Error("Missing arguments");
  const projectPath = await resolveProjectPath(vaultPath, args.project as string);
...
export async function addSessionSummary(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
  if (!args) throw new Error("Missing arguments");
  const projectPath = await resolveProjectPath(vaultPath, args.project as string);
...
export async function getResumeContext(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
  if (!args) throw new Error("Missing arguments");
  const projectPath = await resolveProjectPath(vaultPath, args.project as string);
...
export async function addDecision(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
  if (!args) throw new Error("Missing arguments");
  const projectPath = await resolveProjectPath(vaultPath, args.project as string);
...
export async function getDecision(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
  if (!args) throw new Error("Missing arguments");
  const projectPath = await resolveProjectPath(vaultPath, args.project as string);
  const decisionsPath = path.join(projectPath, "Decisions");
...
export async function closeDecision(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
  if (!args) throw new Error("Missing arguments");
  const projectPath = await resolveProjectPath(vaultPath, args.project as string);
  const decisionsPath = path.join(projectPath, "Decisions");
  const idStr = String(args.id as number).padStart(3, "0");
...
export async function supersedeDecision(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
  if (!args) throw new Error("Missing arguments");
  const projectPath = await resolveProjectPath(vaultPath, args.project as string);
  const decisionsPath = path.join(projectPath, "Decisions");
...
`;
  ...
  const newFilePath = path.join(decisionsPath, `DEC-${newIdStr} - ${sanitizeTitle(args.newTitle as string)}.md`);
...
export async function listDecisions(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
  if (!args) throw new Error("Missing arguments");
  const projectPath = await resolveProjectPath(vaultPath, args.project as string);
  const decisionsPath = path.join(projectPath, "Decisions");
...
export async function linkEntity(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
  if (!args) throw new Error("Missing arguments");
  const projectPath = await resolveProjectPath(vaultPath, args.project as string);
...
export async function findProjectByLocalPath(vaultPath: string, args: ToolArgs): Promise<ToolResult> {
  if (!args) throw new Error("Missing arguments");
